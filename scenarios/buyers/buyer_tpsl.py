...
class BuyerTPSL(MarketProcess):
    ...
    def _tick(self, current_time: datetime):
        ...
        if not self.in_position:
            ...
            self._check_exit_conditions(current_price, current_timestamp)
    ...
    def _open_position(self, price: float, timestamp: datetime):
        ...
        print(f"[{timestamp}] BUY OPEN @ {price:.2f} | "
              ...
              f"spent: {cost:.2f} + fee {fee_cost:.2f}")
    ...
    def _check_exit_conditions(self, current_price: float, timestamp: datetime):
        ...
        if hit_tp or hit_sl:
            self._close_position(current_price, timestamp, "TP" if hit_tp else "SL")
