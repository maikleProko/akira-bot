...
class BuyerKAMA(MarketProcess):
    def __init__(
            ...
            fee: float = 0.001,
            investment_usdt: float = 1000.0,
            stop_loss_usdt: float = -10.0
    ):
        self.history_market_parser = history_market_parser
        self.kama_indicator = kama_indicator
        self.kama_indicator_other = kama_indicator_other
        self.kama_indicator_other2 = kama_indicator_other2
        ...
        self.stop_loss_usdt = stop_loss_usdt
        ...
        self._log(
            f"\n=== НОВАЯ СЕССИЯ | {symbol1}/{symbol2} | "
            f"Баланс: {balance_usdt.amount:.2f} USDT ===\n"
        )
    ...
    def _fix_deal(self, reason: str):
        with open("files/decisions/deals.txt", "a", encoding="utf-8") as f:
            f.write("{")
    ...
    # --------------------------------------------------
    def _current_net_pnl(self, price: float) -> float:
        amount = self.amount_in_position

        entry_cost = self.entry_price * amount
        entry_fee = entry_cost * self.fee
        total_entry = entry_cost + entry_fee

        exit_gross = amount * price
        exit_fee = exit_gross * self.fee
        exit_net = exit_gross - exit_fee

        return exit_net - total_entry
    ...
    def _tick(self, current_time: datetime):
        ...
        # ✅ STOP-LOSS ПО LOW СВЕЧИ
        if self.in_position:
            worst_pnl = self._current_net_pnl(low_price)

            if worst_pnl <= self.stop_loss_usdt:
                self._log(
                    f"STOP LOSS HIT (LOW) | "
                    f"worst PnL: {worst_pnl:.2f} USDT"
                )
        ...
        if not self.in_position:
            if (
                ...
                and self.kama_indicator.trend2 == "BULLISH"
                and self.kama_indicator.trend3 == "BULLISH"
                and self.kama_indicator_other.trend == "BULLISH"
                and self.kama_indicator_other2.trend == "BULLISH"
            ):
                self._open_position(close_price, self.current_timestamp)
    ...
    def _open_position(self, price: float, timestamp: datetime):
        ...
        self.amount_in_position = amount
        ...
        self._log(
            f"BUY OPEN @ {price:.2f} | "
            f"amount: {amount:.6f} {self.symbol1} | "
    ...
    def _close_position(self, price: float, timestamp: datetime, reason: str):
        ...
        entry_cost = self.entry_price * amount
        entry_fee = entry_cost * self.fee
        net_pnl = proceeds_net - (entry_cost + entry_fee)

        self._log(
            f"CLOSE {reason} @ {price:.2f} | "
