...
class CHoCHStrategy(Strategy):
    ...
    def run_historical(self, start_time, current_time):
        if (
                self.choch_indicator.is_now_CHoCH and self.choch_indicator.choch_cross_price <= self.history_market_parser_1m.df['close'].iloc[-1] or
                self.bos_indicator.is_now_BOS and self.bos_indicator.bos_cross_price <= self.history_market_parser_1m.df['close'].iloc[-1]
        ) and \
