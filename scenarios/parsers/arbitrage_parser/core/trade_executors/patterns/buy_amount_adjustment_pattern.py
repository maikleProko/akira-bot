...
class BuyAmountAdjustmentPattern(ValidationPattern):
    ...
    def log_adjusted_buy(self, adjusted_amount_dec, from_asset, quote_increment_dec, adjusted_base_amount_dec, to_asset, current_ask_price_dec):
        self.logger.log_message(f"Скорректированная сумма для покупки: {adjusted_amount_dec} {from_asset} (quote_increment: {quote_increment_dec}, expected {adjusted_base_amount_dec} {to_asset} at price {current_ask_price_dec})")
    ...
    def adjust_buy_amount(self, funds, current_ask_price, base_min_size, base_increment, quote_increment, to_asset, symbol, available, from_asset):
        ...
        if not success:
            return 0, False
        self.log_adjusted_buy(adjusted_amount_dec, from_asset, quote_increment_dec, adjusted_base_amount_dec, to_asset, current_ask_price)
        return float(adjusted_amount_dec), True

    def validate(self, funds, current_ask_price, base_min_size, base_increment, quote_increment, to_asset, symbol, available, from_asset):
        return self.adjust_buy_amount(funds, current_ask_price, base_min_size, base_increment, quote_increment, to_asset, symbol, available, from_asset)