...
class TradeExecutor:
    ...
    def adjust_for_sell(self, amount, direction, base_min_size, base_increment, from_asset, symbol, adjusted_amount_mode):
        if direction == 'sell':
            adjusted_amount, success = self.trade_validator.adjust_sell_amount(self.logger.log_message, amount, base_min_size, base_increment, from_asset, symbol, adjusted_amount_mode)
            #if not success:
                #return None, False
            return adjusted_amount, True
        return amount, True

    def adjust_for_buy(self, amount, direction, quote_min_size, from_asset, symbol, current_ask_price, base_min_size, base_increment, quote_increment, to_asset, adjusted_amount_mode):
        if direction != 'sell':
            ...
            #if not success:
                #return None, False
            adjusted_amount, success = self.trade_validator.adjust_buy_amount(self.logger.log_message, funds, current_ask_price, base_min_size, base_increment, quote_increment, to_asset, symbol, amount, from_asset, adjusted_amount_mode)
            #if not success:
                #return None, False
            return adjusted_amount, True
        return amount, True

    def execute_trade_prod(self, from_asset, to_asset, amount, symbol, direction, expected_price, fee_rate, price_map, base_min_size, quote_min_size, base_increment, quote_increment, ask_price, adjusted_amount_mode):
        start_time = time.time()
        try:
            adjusted_amount, success = self.adjust_for_sell(amount, direction, base_min_size, base_increment, from_asset, symbol, adjusted_amount_mode)
            if not success:
                return None, False
            if direction != 'sell':
                ...
                if current_ask_price is None:
                    return None, False
                adjusted_amount, success = self.adjust_for_buy(amount, direction, quote_min_size, from_asset, symbol, current_ask_price, base_min_size, base_increment, quote_increment, to_asset, adjusted_amount_mode)
                if not success:
                    return None, False
    ...
    def execute_trade(self, from_asset, to_asset, amount, symbol, direction, expected_price, fee_rate, price_map):
        ...
        if not success:
            return None, False
        return self.execute_trade_prod(from_asset, to_asset, amount, symbol, direction, expected_price, fee_rate, price_map, base_min_size, quote_min_size, base_increment, quote_increment, ask_price, 0)
