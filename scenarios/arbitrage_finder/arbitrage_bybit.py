...
def fetch_instruments(category="spot"):
    """
    Возвращает список инструментов (список словарей) из /v5/history_market/instrument.
    Учитывает разные обёртки ответа (result.list / result.data и т.п.).
    """
    url = BYBIT_API + "/v5/history_market/instrument"
...
def fetch_tickers(category="spot"):
    """
    Возвращает список тикеров из /v5/history_market/tickers (result.list или result.data).
    """
    url = BYBIT_API + "/v5/history_market/tickers"
    params = {"category": category}
    j = get_json(url, params=params)
    res = j.get("result", j)
    if isinstance(res, dict):
