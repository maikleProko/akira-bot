...
from scenarios.unused_scenarios.generate_order_book_data.constants.OrderBookParserConstants import *
...
class PaneImporter:
    ...
    def import_order_book_data(self, image_path):
        ...
        self.order_book_data = rectangles

    def fill_gaps(self):
        if not self.order_book_data:
            return []

        rects_sorted = sorted(deepcopy(self.order_book_data), key=lambda r: (r['y1'], r['x1']))
        result = []
        ...
        self.order_book_data = result

    def restore_order_book_data(self, top_border, bottom_border, round_digits=None, inplace=False):
        ...
        output = self.order_book_data if inplace else deepcopy(self.order_book_data)
        ...
        self.order_book_data = output

    def get_order_book_data(self):
        self.import_order_book_data(INPUT_PANE_FILE)
        self.fill_gaps()
