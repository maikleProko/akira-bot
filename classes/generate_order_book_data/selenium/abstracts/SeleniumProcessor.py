...
class SeleniumProcessor:
    ...
    def write(self, css_selector, value):
        self.get_element(By.CSS_SELECTOR, css_selector).send_keys(value)
        logger.info("Was written {} on element with css_celector {}".format(value, css_selector))
    ...
    def send_key_combination_selector(self, css_selector, modifier_key, regular_key):
        element = self.get_element(By.CSS_SELECTOR, css_selector)
        self.send_key_combination_element(element, modifier_key, regular_key)

    def send_key_combination_class(self, class_name, modifier_key, regular_key):
        element = self.get_element(By.CLASS_NAME, class_name)
        self.send_key_combination_element(element, modifier_key, regular_key)

    def send_key_combination_xpath(self, xpath, modifier_key, regular_key):
        element = self.get_element(By.XPATH, xpath)
        self.send_key_combination_element(element, modifier_key, regular_key)

    def send_key_combination_element(self, element, modifier_key, regular_key):
    ...
    def screen_selector(self, css_selector, filename, offset: int = 0):
        print('123')
        element = self.get_element(By.CSS_SELECTOR, css_selector)
        self.screen(element, filename, offset)

    def screen_class(self, class_name, filename, offset: int = 0):
        element = self.get_element(By.CLASS_NAME, class_name)
        self.screen(element, filename, offset)

    def screen_xpath(self, xpath, filename, offset: int = 0):
        element = self.get_element(By.XPATH, xpath)
        self.screen(element, filename, offset)

    def screen(self, element, filename: str, offset: int = 0) -> bool:
        try:
            # Получаем размеры элемента
            size = element.size
            # Создаем новый прямоугольник с учетом offset
            location = element.location
            # Создаем скриншот всего элемента
            element.screenshot(filename)

            # Открываем изображение и обрезаем его
            ...
            logger.error("Element {} isnt screened:".format(element))
            logger.error(e)
            return False
